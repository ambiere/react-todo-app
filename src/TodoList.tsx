    ...
    return (
        <div className='w-[640px] mr-auto ml-auto flex flex-col'>
            <form action="" className='flex flex-col md: w-8/12 mr-auto ml-auto' onSubmit={handleSubmit}>
                <input type="text"
                    value={todoText}
                    className='outline-none border h-9 p-2 mb-4 text-sm'
                    placeholder='Enter To Do'
                    onChange={(event) => setTodoText(event.target.value)}
                />
                <button type="submit"
                    className='bg-blue-600 text-white font-semibold w-12 h-8 rounded-[4px] mb-6'>
                    {buttonTitle}
                </button>
            </form>
            ...
            <ul className='md: w-8/12 mr-auto ml-auto'>
                {state.todos.map(todo => (
                    <li key={todo.id} className='grid gap-2 p-1 m-0 grid-cols-8 border-b-[1px]'>
                        ...
                        <span className='cursor-pointer text-sm text-blue-600 underline' onClick={() => {
                            setTodoText(todo.text)
                            setEditMode(true)
                            setEditTodo(todo)
                        }}>Edit</span>
                        <span className='cursor-pointer text-sm text-red-600 underline ' onClick={async () => {
